...
import type { ClaudopilotConfig, RepoConfig, RedTeamConfig, ImproveConfig, CompetitorsConfig, DreamConfig, MockupConfig, AssigneeConfig, AutoApproveConfig } from "../types.js";
...
export async function installClaudeCommands(
  ...
  if (config.mockups?.enabled) {
    await writeFile(
      join(commandsDir, "mockup.md"),
      generateMockupCommand(config)
    );
  }
...
function generateMockupCommand(config: ClaudopilotConfig): string {
  const maxFiles = config.mockups?.maxFiles ?? 10;
  const owner = config.github.owner;
  const repo = config.github.repos[0];

  return `You are generating UI wireframe mockups for a spec file.

INPUT: $ARGUMENTS (path to a spec file, e.g. specs/my-feature.md)

Read the spec file at $ARGUMENTS to understand the proposed UI changes.

STACK DETECTION — exactly 3 file reads, no more:
1. Read \`package.json\` to identify the framework (React, Vue, Svelte, etc.) and CSS approach (Tailwind, CSS modules, etc.)
2. Read the first matching CSS config file: \`tailwind.config.*\`, \`postcss.config.*\`, or \`vite.config.*\` (skip if none found)
3. Read ONE sample component file: the first file matching \`src/components/**/*.{tsx,vue,svelte}\` or \`src/app/**/*.tsx\` (skip if none found)

Do NOT read additional files beyond these 3. The wireframes use a standard style regardless of what framework is detected.

GENERATE WIREFRAME HTML FILES:

For each distinct screen or state described in the spec, generate a self-contained HTML file.

Rules for each HTML file:
- Self-contained: all CSS in a \`<style>\` block, NO external resources, NO JavaScript
- Wireframe style ONLY — monochrome gray palette:
  - Backgrounds: #f5f5f5
  - Placeholder boxes: #e0e0e0
  - Text: #333
  - Borders: 1px solid #ccc
- System fonts only: \`-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif\`
- Box-and-label layout style (rectangles with labels, not pixel-perfect designs)
- Every file MUST include this sticky header at the top:
  \`<div style="position:sticky;top:0;background:#fff3cd;border-bottom:2px solid #ffc107;padding:12px 20px;text-align:center;font-weight:bold;z-index:1000;">⚠️ WIREFRAME — Layout Only, Not Final Design</div>\`
- Annotate with HTML comments explaining each section's purpose
- Responsive via flexbox/grid (no fixed widths)
- Maximum ${maxFiles} files per feature

Save files to: \`specs/mockups/<feature-name>/\`
where \`<feature-name>\` is derived from the spec filename (e.g. \`specs/my-feature.md\` → \`specs/mockups/my-feature/\`)

GENERATE index.html — a navigation page listing all generated mockups with descriptions of what each one shows.

GENERATE PREVIEW LINKS:

After generating files, determine the current git branch:
\`git rev-parse --abbrev-ref HEAD\`

For each mockup file, construct preview URLs in this format:
\`https://htmlpreview.github.io/?https://github.com/${owner}/${repo}/blob/<branch>/specs/mockups/<feature>/<filename>\`

NOTE FOR PRIVATE REPOS: \`htmlpreview.github.io\` may not work for private repositories. If the repo is private, provide the GitHub blob URL instead with instructions: "Open the link, click 'Raw', then save the page and open locally."

ADD TO SPEC: Append a \`## Visual Mockups\` section to the spec file ($ARGUMENTS) with:
- List of generated files
- Preview URLs for each
- Note about the wireframe-only nature of the mockups

OUTPUT: Print a markdown summary listing all generated files with their preview URLs.
`;
}
